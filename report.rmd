...
##
```{r, include = FALSE}
spotify_data <- read_xlsx("spotify_2023_data.xlsx")
```
...
p <- ggplot(spotify_data, aes(x = streams)) +
        geom_histogram(binwidth = 100000000, fill = "#1DB954", color = "#008222", alpha = 1000) +
        xlab("Streams") +
...
## Correlation matrix
```{r}
numeric <- spotify_data %>% select_if(is.numeric)
gr <- colorRampPalette(c("black", "white", "#1DB954"))
corPlot(numeric, gr = gr, main = "")
```

##
```{r}
gr2 <- colorRampPalette(c("black", "white", "#1DB954"))
corPlot(numeric[, 5:12], gr = gr2, main = "", upper = FALSE)
```
...
p11 <- ggplot(spotify_data, aes(x = in_spotify_playlists)) +
  geom_histogram(binwidth = 1000, fill = "#1DB954", color = "#008222", alpha = 1000) +
  xlab("Spotify playlists") +
  ylab("Number of songs") +
  theme(plot.title = element_text(size = 15)) +
  theme_bw()
...
p33 <- ggplot(spotify_data, aes(x = in_apple_playlists)) +
  geom_histogram(binwidth = 20, fill = "#1DB954", color = "#008222", alpha = 1000) +
  xlab("Apple Music playlists") +
  ylab("Number of songs") +
  theme(plot.title = element_text(size = 15)) +
  theme_bw()
...
p4 <- ggplot(spotify_data, aes(x = in_spotify_charts)) +
  geom_histogram(binwidth = 10, fill = "#1DB954", color = "#008222", alpha = 1000) +
  xlim(1, 150) +
  xlab("Spotify charts - only ranked") +
  ylab("Number of songs") +
  theme(plot.title = element_text(size = 15)) +
  theme_bw()
...
p6 <- ggplot(spotify_data, aes(x = in_apple_charts)) +
  geom_histogram(binwidth = 10, fill = "#1DB954", color = "#008222", alpha = 1000) +
  xlim(1, 290) +
  xlab("Apple Music charts - only ranked") +
  ylab("Number of songs") +
  theme(plot.title = element_text(size = 15)) +
  theme_bw()
...
p5 <- ggplot(spotify_data, aes(x = in_deezer_charts)) +
  ...
  xlim(1, 50) +
  ylim(0, 100) +
  xlab("Deezer charts - only ranked") +
  ylab("Number of songs") +
  theme(plot.title = element_text(size = 15)) +
  theme_bw()
...
## MODELS
* Model 3A: $Streams_i = \beta_0 + \beta_1 \times Sch + \beta_2 \times Ach + \beta_3 \times Dch \\
+ \beta_4 \times (Sch \times Ach)_i + \beta_5 \times (Ach \times Dch)_i + \beta_6 \times (Sch \times Dch)_i
 + \beta_7 \times (Sch \times Dch \times Ach)_i + \epsilon_i$
